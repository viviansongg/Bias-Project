...
def collect_url_extensions(urls):
    """
    Collects the extensions from a list of URLs.

    Args:
        urls (list of str): List of URLs to process.

    Returns:
        list of str: List of extracted URL extensions.
    """
    extensions = []
    for url in urls:
...
extensions = collect_url_extensions(urls)
print(extensions)
...
credibility_scores = evaluate_urls(urls)
for url, score in credibility_scores:
    print(f"URL: {url}, Credibility Score: {score}")